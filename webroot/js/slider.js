...
{	
	...
	if (cnt == 1)
	{
		var header_color = $('#header_image1').attr('color');		
		$('#header_image1').show();
	}
	else if (cnt == 2)
	{
		var header_color = $('#header_image2').attr('color');		
		$('#header_image2').show();
	}
	else if (cnt == 3)
	{
		var header_color = $('#header_image3').attr('color');		
		$('#header_image3').show();
	}
	else
	{
		alert('bad count, stopping: ' + cnt);
		return;
	}
	
     $('#main-slider').css({'background-image':'none','background-color':header_color});

	cnt++;
	if (cnt > header_count)
		cnt = 1;
		
	if (interval_header != null)
		clearInterval(interval_header);
	
	interval_header = setInterval(timerFunction, 5000);	
